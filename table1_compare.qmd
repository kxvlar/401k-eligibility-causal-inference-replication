...
build_dat <- function(t3, t6, t9, t12, t7, w7, w8, w9) {
 ...
 dat <- dat %>%
  mutate(
    ...
    # tweak: require regression vars (except hh_inc_year) to be non-missing
...
```
```{r}
# Rebuild from scratch using existing build_dat()
dat_raw <- build_dat(
  t3_raw, t6_raw, t9_raw, t12_raw,
  t7_raw, w7_raw, w8_raw, w9_raw
)
...
# Check ID sets really match
raw_ids <- dat_raw_id %>% select(all_of(id_vars)) %>% distinct()
rep_ids <- dat_rep_id %>% select(all_of(id_vars)) %>% distinct()

rep_not_raw <- dplyr::anti_join(rep_ids, raw_ids, by = id_vars)
raw_not_rep <- dplyr::anti_join(raw_ids, rep_ids, by = id_vars)

nrow(rep_not_raw)  # should be 0
nrow(raw_not_rep)  # should be 0

...
main <- dat_raw_aligned  # ← NOW USING dat_raw_aligned, but switch to data_raw to use 1115 sample size
...
main <- main %>%
  ...
  ## apply paper’s required sample restrictions
  filter(
    age_ok,
    for_profit,
    yr1jb1
...
table2_raw_aligned <- bind_rows(table2_results)
table2_raw_aligned
...
# Merge side-by-side by outcome + spec
table2_compare <- t_rep %>%
  inner_join(t_raw, by = c("outcome", "spec")) %>%
